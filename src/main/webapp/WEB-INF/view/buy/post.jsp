			...
			$.each(buys, (i, buy) => {
				buyList.unshift(
					`<li class='row border-bottom pt-3  pb-3 align-items-center'>
						...
						<a class='col ml-1' href='buy/view?tradeNum=\${buy.tradeNum}'>
							<strong class='d-block w-100'>[\${buy.tradeCode}] \${buy.title}</strong>
							<span class='d-block'>
								<small class='small pr-3 border-right'>\${buy.location}</small>
								<time class='pl-3'><small>\${buy.tradeRegDate}</small></time>
							</span>
							<span class='price d-block'>\${buy.price}</span>
						</a>
					</li>`)
			})
			
			$('#buyList').append(buyList.join(''));
		} else {
			$('#buyList').empty()
			$('#buyList').append('<li class="text-center mt-3">거래중인 상품이 없습니다.</li>')
		}
	})		
}
...
<body>
	...
	<div class='container min-vh-100'>
		...
		<div class='row m-1 justify-content-between pt-3 border-top'>
			...
			<a href='buy/write' class='btn form-control sellBtn col border ml-2 text-white'>글 작성</a>
		</div>

		<ul id='buyList' class='col list-unstyled mt-3 border-top'>
